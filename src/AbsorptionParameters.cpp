...
void AtmosphericAbsorptionParameters(double f__mhz, double* gamma_oo, double* gamma_ow)
{
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        if (f__mhz < freqs__mhz[i])
        {
            ...
            double R = (log10(f__mhz) - log10(freqs__mhz[i - 1])) / (log10(freqs__mhz[i]) - log10(freqs__mhz[i - 1]));

            double X = (R * (log10(oxygen_data[i]) - log10(oxygen_data[i - 1]))) + log10(oxygen_data[i - 1]);
            *gamma_oo = pow(10.0, X);
            ...
            {
                double Y = (R * (log10(water_data[i]) - log10(water_data[i - 1]))) + log10(water_data[i - 1]);
                *gamma_ow = pow(10.0, Y);
            }
