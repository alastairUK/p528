...
{
    double alpha__rad = (PI / 2.0) + beta__rad;
    double z_T__km = a__km + T_e__km;
    ...
    else if (z_T__km < z_1__km)
    {
        ...
        {
            double z_c__km = z_1__km * sin(alpha__rad);
            if (z_T__km <= z_c__km)
                r_e__km = 0.0;
            else
    ...
    {
        double A_q__rad = asin(z_1__km * sin(alpha__rad) / z_T__km);
        double A_e__rad = PI - (alpha__rad + A_q__rad);
